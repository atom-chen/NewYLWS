...
namespace Cinemachine
{
    ...
    public class CinemachineTargetGroup : MonoBehaviour
    {
        ...
        public void AddTarget(Transform targetTrans, float weight, float radius)
        {
            Target target = new Target();
            target.target = targetTrans;
            ...
            if (!added)
            {
                Target[] targetList = new Target[(m_Targets.Length + 10) * 2];
                System.Array.Copy(m_Targets, targetList, m_Targets.Length);
                targetList[m_Targets.Length] = target;
        ...
        public void RemoveTarget(Transform targetTrans)
        {
            for (int i = 0; i < m_Targets.Length; ++i)
            {
                if (m_Targets[i].target == targetTrans)
                {
                    m_Targets[i].target = null;
                    return;
                }
            }
        }
