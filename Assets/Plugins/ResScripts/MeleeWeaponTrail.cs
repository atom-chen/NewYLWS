		...
		if (pointsToUse.Count > 1)
		{
			...
			for (int n = 0; n < pointsToUse.Count; ++n)
			{
				...
				if (_colors != null && _colors.Length > 0)
				{
                    ...
                    if (minInt >= _colors.Length) minInt = _colors.Length - 1; if (minInt < 0) minInt = 0;
                    if (maxInt >= _colors.Length) maxInt = _colors.Length - 1; if (maxInt < 0) maxInt = 0;
                    color = Color.Lerp(_colors[minInt], _colors[maxInt], lerp);
				}
				...
				if (_sizes != null && _sizes.Length > 0)
				{
                    ...
                    int minInt = (int)min;
                    int maxInt = (int)max;
                    if (minInt >= _sizes.Length) minInt = _sizes.Length - 1; if (minInt < 0) minInt = 0;
                    if (maxInt >= _sizes.Length) maxInt = _sizes.Length - 1; if (maxInt < 0) maxInt = 0;
                    size = Mathf.Lerp(_sizes[minInt], _sizes[maxInt], lerp);
				}
