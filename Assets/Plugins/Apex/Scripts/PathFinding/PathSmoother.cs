...
namespace Apex.PathFinding
{
    ...
    public class PathSmoother : ISmoothPaths
    {
        ...
        private static bool CanReducePath(IPositioned point1, IPositioned point3, AttributeMask requesterAttributes, float requesterRadius, CellMatrix matrix)
        {
            ...
            var startCell = matrix.GetCell(p1 - radiusAdjust, false);
            if (startCell == null)
            {
                startCell = matrix.GetCell(p1, true);
            }

            var endCell = matrix.GetCell(p3 + radiusAdjust, false);
            if (endCell == null)
            {
                endCell = matrix.GetCell(p3, true);
            }
