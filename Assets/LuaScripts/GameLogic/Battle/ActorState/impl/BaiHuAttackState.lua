...
local BaiHuAttackState = BaseClass("BaiHuAttackState", AttackState)
local SAnimationState = require "GameLogic.Battle.Animator.SAnimator"

function BaiHuAttackState:InitAnim()
    ...
    local skillAnimCfg = GetAnimationCfgByName('3503_skl35034')
    if skillAnimCfg then
        local state = SAnimationState.New()
        for _, t in ipairs(skillAnimCfg.keyframe) do
            state:AddEventTime(t)
...
-- function BaiHuAttackState:DoAtk(target)
