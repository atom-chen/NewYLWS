...
local LieZhuanLogic = BaseClass("LieZhuanLogic", BaseBattleLogic)

local base = BaseBattleLogic

function LieZhuanLogic:__init()
    self.m_leftPosList = {
        NewFixVector3(0, 0, 0),
    ...
    self.m_battleType = BattleEnum.BattleType_LIEZHUAN
    self.m_copyCfg = false
end

function LieZhuanLogic:OnEnterParam(enterParam)
    base.OnEnterParam(self, enterParam)
    self:InitCopyCfg()
end

function LieZhuanLogic:InitCopyCfg()
    self.m_copyCfg = ConfigUtil.GetLieZhuanCopyCfgByID(self.m_battleParam.copyID)
end

function LieZhuanLogic:OnBattleInit()
...
function LieZhuanLogic:FlushMonster(immediatelyCreateObj)
...
function LieZhuanLogic:FlushBattleRound(battleRoundCfg, immediately)
...
function LieZhuanLogic:CreatePlot()
    self.m_plotContext = SequenceMgr:GetInstance():PlayPlot('PlotLieZhuan')
end

function LieZhuanLogic:GetLeftPos(wave)
    return self.m_leftPosList
end

function LieZhuanLogic:GetRightPos(wave)
    if wave <= 0 then
        return nil
    end
...
function LieZhuanLogic:GoToCurrentWaveStandPoint(ignorePartner)
...
function LieZhuanLogic:PlayDollyGroupCamera(dollyImmediate)
...
function LieZhuanLogic:GetWaveGoTimelineName()
...
function LieZhuanLogic:GetGoWaveTimelinePath()
    return self.m_mapCfg.timelinePath
end

function LieZhuanLogic:OnNextWaveArrived()  
    SequenceMgr:GetInstance():TriggerEvent(SequenceEventType.BATTLE_GO_END)
end


function LieZhuanLogic:OnActorDie(actor, killerGiver, hurtReason, deadMode)
    if not actor then
        return
    end
...
function LieZhuanLogic:Update(deltaMS, battlestatus)
...
function LieZhuanLogic:DoFinish()
    base.DoFinish(self)
...
function LieZhuanLogic:ReqSettle(isWin)
...
function LieZhuanLogic:RecordCommand()
    return true
end

return LieZhuanLogic
