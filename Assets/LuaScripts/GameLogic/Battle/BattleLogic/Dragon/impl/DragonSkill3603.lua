...
local DragonSkill3603 = BaseClass("DragonSkill3603", DragonSkillBase)

function DragonSkill3603:PerfromDragonSkill(battleDragon)
    if battleDragon:GetEffectIndex() == 0 then
        ...
        local mediumParam = {
            ...
            phyHurt = self:X(),
            phyHurtPercent = FixDiv(self:Y(), 100),
        }
        ...
        MediumManagerInst:CreateMedium(MediumEnum.MEDIUMTYPE_3603_DAZHAO, 360301, giver, self, fireBallStartPos, FixNewVector3(1, 0, 0), mediumParam)

        self:CheckDragonTalentSkill()
        
        AudioMgr:PlayAudio(7004)
