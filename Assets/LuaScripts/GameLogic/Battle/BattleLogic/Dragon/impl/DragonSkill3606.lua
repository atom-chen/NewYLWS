...
local DragonSkill3606 = BaseClass("DragonSkill3606", DragonSkillBase)

function DragonSkill3606:PerfromDragonSkill(battleDragon)
    if battleDragon:GetEffectIndex() == 0 then
        local center = self:GetEnemyCampCenterByID(battleDragon:GetCamp())
        local mediumParam = {
            effectPos = Vector3.New(center.x, center.y, center.z),
            camp = battleDragon:GetCamp(),
            freezeTime = self:Y(),
            defChgPercent = self:X(),
            dragonLevel = self.m_dragonLevel,
        }
        ...
        MediumManagerInst:CreateMedium(MediumEnum.MEDIUMTYPE_3606_DAZHAO, 360601, giver, self, fireBallStartPos, FixNewVector3(1, 0, 0), mediumParam)
        
        self:CheckDragonTalentSkill()
        
        AudioMgr:PlayAudio(7008)
