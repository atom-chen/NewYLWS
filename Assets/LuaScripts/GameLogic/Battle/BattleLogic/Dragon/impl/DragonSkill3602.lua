...
local DragonSkill3602 = BaseClass("DragonSkill3602", DragonSkillBase)

function DragonSkill3602:PerfromDragonSkill(battleDragon)
    if battleDragon:GetEffectIndex() == 0 then
        ...
        local mediumParam = {
            ...
            magicHurt = self:X(),
            magicHurtPercent = FixDiv(self:Y(), 100),
        }
        ...
        MediumManagerInst:CreateMedium(MediumEnum.MEDIUMTYPE_3602_DAZHAO, 360201, giver, self, fireBallStartPos, FixNewVector3(1, 0, 0), mediumParam)

        self:CheckDragonTalentSkill()

        AudioMgr:PlayAudio(7006)
