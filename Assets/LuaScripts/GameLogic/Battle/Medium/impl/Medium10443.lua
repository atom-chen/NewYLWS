...
local Medium10443 = BaseClass("Medium10443", LinearFlyToTargetMedium)
...
function Medium10443:InitParam(param, index)
...
function Medium10443:DoUpdate(deltaMS)
    ...
    elseif self.m_mediumState == MediumState.FlyAway then
        if not self:IsAtOwnerForward(owner) or not owner:CanFlyAway() then 
            self:SurroundOwner(owner, deltaMS)
            return
        end

        if self.m_currentTargetID <= 0 then
            self.m_currentTargetID = owner:RandEnemyActorID()
        end
...
function Medium10443:ArriveDest()
    self:Hurt()
end

function Medium10443:SurroundOwner(owner, deltaMS)
    local speed = 1.5
...
function Medium10443:IsAtOwnerForward(owner)
...
function Medium10443:Hurt()
    local performer = self:GetOwner()
    if not performer then
        return
    end

    performer:DecGhostCount()
