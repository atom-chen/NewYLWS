...
local Medium22002 = BaseClass("Medium22002", LinearFlyToTargetMedium)

function Medium22002:ArriveDest()
    self:Hurt()
end

function Medium22002:Hurt()
    ...
    --发射飞刀对当前目标造成<color=#ffb400>{x1}%</color>的物理伤害，
    --并令其失明，命中下降<color=#ffb400>{y1}%</color>,持续<color=#1aee00>{A}</color>秒 并令下次普攻可以溅射<color=#1aee00>{B}</color>米内的敌人
    ...
    local level = self.m_skillBase:GetLevel() 
    if level >= 3 then
        performer:Launch22002TakeAtk()
    end 
