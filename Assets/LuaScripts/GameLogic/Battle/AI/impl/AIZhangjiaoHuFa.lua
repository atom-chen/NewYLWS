...
function AIZhangjiaoHuFa:AI(deltaMS)
    ...
    if self.m_onBorn and self.m_currTargetActorID > 0 then
        self.m_onBorn = false
        local skillItem = self.m_selfActor:GetSkillContainer():GetActiveByID(20971)
        if skillItem then
            local atkTarget = ActorManagerInst:GetActor(self.m_currTargetActorID)
            if atkTarget and atkTarget:IsLive() then
                local p = atkTarget:GetPosition()
                self:PerformSkill(atkTarget, skillItem, p, SKILL_PERFORM_MODE.AI)
            end
        end
...
function AIZhangjiaoHuFa:SetAtkTargetID(targetID) -- 普攻集火对象
    self.m_atkTargetID = targetID
end

function AIZhangjiaoHuFa:OnBorn(targetID)
