...
local Skill20012 = BaseClass("Skill20012", SkillBase)

function Skill20012:Perform(performer, target, performPos, special_param)
    ...
    if special_param.keyFrameTimes == 2 then
        ...
        if injure > 0 then
            ...
            local giver = StatusGiver.New(performer:GetActorID(), 20012)
...
function Skill20012:SelectSkillTarget(performer, target)
...
function Skill20012:CheckPerform(performer, target)
    local dis2 = (performer:GetPosition() - target:GetPosition()):SqrMagnitude()
    if dis2 < FixMul(self:A(), self:A()) then
        if self:SelectSkillTarget(performer, target) then
