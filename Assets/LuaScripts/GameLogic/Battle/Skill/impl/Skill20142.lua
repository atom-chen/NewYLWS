...
local Skill20142 = BaseClass("Skill20142", SkillBase)

function Skill20142:Perform(performer, target, performPos, special_param)
    ...
    local giver = StatusGiver.New(performer:GetActorID(), 20142)
    ...
    if injure > 0 then
        local giver = StatusGiver.New(performer:GetActorID(), 20142)
        local intervalHpStatus = StatusFactoryInst:NewStatusIntervalHP(giver, FixMul(-1, injure), 1000, self:A(), nil, 0, BattleEnum.HURTTYPE_MAGIC_HURT)
    ...
    if self.m_level >= 2 then
        ...
        local giver = StatusGiver.New(performer:GetActorID(), 20142) 
        local statusHurtOtherMul = StatusFactoryInst:NewStatusNTimeHurtOtherMul(giver, FixMul(self:A(), 1000), hurtTypeList, {21017})  
