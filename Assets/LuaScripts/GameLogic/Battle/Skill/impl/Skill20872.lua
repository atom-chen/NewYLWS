...
local Skill20872 = BaseClass("Skill20872", SkillBase)

function Skill20872:Perform(performer, target, performPos, special_param)
        ...
        function(tmpTarget)
            if battleLogic:IsFriend(performer, tmpTarget, true) then
                ...
                if curDis <= FixMul(self:A(), self:A()) then
                    local giver = StatusGiver.New(performer:GetActorID(), 20872)
