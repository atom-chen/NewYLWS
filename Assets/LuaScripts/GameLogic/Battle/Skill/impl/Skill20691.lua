...
local Skill20691 = BaseClass("Skill20691", SkillBase)

function Skill20691:Perform(performer, target, performPos, special_param)
    if not performer or not performer:IsLive() or not target or not target:IsLive() then
        return
    end
    
    -- 水之禁锢
    -- 吟唱{A}秒，禁锢目标，每秒造成{x1}%的法术伤害，持续{B}秒。
    -- 吟唱{A}秒，禁锢目标，每秒造成{x2}%的法术伤害，持续{B}秒，该效果不可驱散。
    ...
    if injureInterval > 0 then
        ...
        local giver = statusGiverNew(performer:GetActorID(), 20691)
