...
local function OnCreate(self, wrap_class, ...)
	...
	-- Unity侧原生组件
	self.unity_scrollrect = self.transform:GetComponentInParent(Type_ScrollRect)
	self.unity_grid = self.transform:GetComponent(Type_GridLayoutGroup)
	self.unity_sizefitter = self.transform:GetComponent(Type_ContentSizeFitter)
	...
	local scroll_rect_trans = self.unity_scrollrect.transform:GetComponent(Type_RectTransform)
