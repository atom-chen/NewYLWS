...
local UIUtil = UIUtil
local Type_RectTr = typeof(CS.UnityEngine.RectTransform)
...
local function OnCreate(self)
	...
	if self._class_type == UILayer then
		...
		while not IsNull(now_holder) do	
			...
			elseif not IsNull(now_holder.view) then
				...
				break
			elseif IsNull(now_holder.holder) then -- UIItem走这里
				self.view = self
				break
			end
	...
	self.rectTransform = UIUtil.FindComponent(self.transform, Type_RectTr)
	self.__var_arg = nil
end
