...
local InscriptionCopyHardItem = BaseClass("InscriptionCopyHardItem", UIBaseItem)
local base = UIBaseItem

function InscriptionCopyHardItem:OnCreate()
    self.m_copyID = 0
...
function InscriptionCopyHardItem:SetData(copyID, locked, isSelected, showEffectBounds)
    ...
    if locked then
        ...
        local preCopyCfg = ConfigUtil.GetInscriptionCopyCfgByID(copyID - 1)
        if preCopyCfg then
...
function InscriptionCopyHardItem:DoSelect(isSelected, showEffectBounds)
...
function InscriptionCopyHardItem:GetCopyID()
    return self.m_copyID
end

function InscriptionCopyHardItem:OnClick(go, x, y)
    if self.m_locked then
        return
    end

    UIManagerInst:Broadcast(UIMessageNames.MN_INSCRIPTION_COPY_CLICK_COPY, self.m_copyID)
end

function InscriptionCopyHardItem:OnDestroy()
...
function InscriptionCopyHardItem:ClearEffect()
...
return InscriptionCopyHardItem