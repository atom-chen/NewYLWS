...
function GuildWarRankView:InitView()
    ...
    self.m_rank3AwardContentTr, self.m_closeBtn = UIUtil.GetChildTransforms(self.transform, {
        "BlackBg",
...
function GuildWarRankView:OnEnable(...)
    ...
    if self.m_toggleList[1] then
        self.m_toggleList[1].isOn = true
    end
    for i = 2, #self.m_toggleList do
        self.m_toggleList[i].isOn = false
    end
...
function GuildWarRankView:UpdateData() 
    ...
    if warBriefData then
        ...
        self.m_upLeftRoundTimeDes1Txt.text = Language.GetString(2347)
...
function GuildWarRankView:OnDestroy()
    ...
    for _, toggle in ipairs(self.m_toggleList) do
        local toggleGo = toggle.gameObject
        if toggleGo then 
            UIUtil.RemoveClickEvent(toggleGo)
        end
    end
    base.OnDestroy(self)
end
