...
local GuildWarMemberItem = BaseClass("GuildWarMemberItem", UIBaseItem)

function GuildWarMemberItem:OnCreate()
    base.OnCreate(self)
...
function GuildWarMemberItem:OnDestroy()
...
function GuildWarMemberItem:OnClick(go, x, y)
    if go.name == "CheckBtn" then
        GuildWarMgr:ReqUserDefBuZhenInfo(self.m_uID, self.m_cityID)
    end
end

function GuildWarMemberItem:InitVariable()
...
function GuildWarMemberItem:InitView()
...
function GuildWarMemberItem:HandleClick()
    local onClick = UILogicUtil.BindClick(self, self.OnClick)

    UIUtil.AddClickEvent(self.m_checkBtn, onClick)
end

function GuildWarMemberItem:UpdateData(guildUserBriefData, isOwn, cityID)
    if guildUserBriefData then

        self.m_cityID = cityID or 0
...
function GuildWarMemberItem:UpdateUserIcon(guildUserBriefData)
