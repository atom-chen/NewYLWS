...
local ShenbingCopyHardItem = BaseClass("ShenbingCopyHardItem", UIBaseItem)
local base = UIBaseItem

function ShenbingCopyHardItem:OnCreate()
    self.m_copyID = 0
...
function ShenbingCopyHardItem:SetData(copyID, locked, isSelected, showEffectBounds)
    ...
    self.m_nameText.text = copyCfg.copyName

    self.m_descText.text = ""

    if locked then
        if Player:GetInstance():GetUserMgr():GetUserData().level < copyCfg.open_level then
            self.m_descText.text = string_format(Language.GetString(1804), copyCfg.open_level)
            return 
        end

        local preCopyCfg = ConfigUtil.GetShenbingCopyCfgByID(copyID - 1)
        if preCopyCfg then
            self.m_descText.text = string_format(Language.GetString(1805), preCopyCfg.copyName)
        end
    else
        local copyCfg = ConfigUtil.GetShenbingCopyCfgByID(copyID)
        if copyCfg then            
...
function ShenbingCopyHardItem:DoSelect(isSelected, showEffectBounds)
...
function ShenbingCopyHardItem:GetCopyID()
    return self.m_copyID
end

function ShenbingCopyHardItem:OnClick(go, x, y)
    if self.m_locked then
        return
    end

    UIManagerInst:Broadcast(UIMessageNames.MN_SBCOPY_CLICK_COPY, self.m_copyID)
end

function ShenbingCopyHardItem:OnDestroy()
...
function ShenbingCopyHardItem:ClearNuqiEffect()
