...
local QingLongAttackState = BaseClass("QingLongAttackState", AttackState)
local SAnimationState = require "GameLogic.Battle.Animator.SAnimator"

function QingLongAttackState:InitAnim()
    ...
    local skillAnimCfg = GetAnimationCfgByName('3501_skl4')
    if skillAnimCfg then
        local state = SAnimationState.New()
        for _, t in ipairs(skillAnimCfg.keyframe) do
            state:AddEventTime(t)
        ...
        state:SetLength(skillAnimCfg.length)
        self.m_skillStates[3] = state
    end
end


-- function QingLongAttackState:DoAtk(target)
--     if SkillUtil.IsActiveSkill(self.m_skillCfg) then
--         if self.m_skillCfg.id == 35014 then
--             self.m_animState = self.m_skillStates[3]
--         else
