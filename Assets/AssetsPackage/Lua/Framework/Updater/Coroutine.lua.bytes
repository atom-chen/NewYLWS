...
-- 协程模块：对Lua协程coroutine进行扩展，使其具有Unity侧协程的特性
-- 注意：
...
local function __RecycleAction(action)
	...
	action.args = false
	action.result = false
	table_insert(action_pool, action)
end
...
local function __GetAction(co, timer, func, args, result)
	...
	if #action_pool > 0 then
		action = table_remove(action_pool)
	else
...
local function __AsyncLoaderCheck(co, async_loader, callback)
	if callback ~= nil then
		callback(co, async_loader:Progress())
	end
	return async_loader:IsDone()
end

-- 等待异步加载完成，并在Update执行完毕resume
-- @async_operation：异步句柄---或者任何带有IsDone、Progress成员属性的异步对象
-- @callback：每帧回调，传入参数为异步加载进度Progress
local function waitforasyncloader(async_loader, callback)
	assert(async_loader)
	local co = coroutine.running() or error ("coroutine.waitforasyncloader must be run in coroutine")
	local timer = TimerManager:GetInstance():GetCoTimer()
	local action = __GetAction(co, timer, __AsyncLoaderCheck, SafePack(co, async_loader, callback), true)
...
coroutine.waitforasyncloader = waitforasyncloader
