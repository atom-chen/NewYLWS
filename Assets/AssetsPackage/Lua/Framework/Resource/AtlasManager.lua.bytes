...
local function LoadImageAsync(self, atlas_config, image_name, callback, ...)
	...
	UIGameObjectLoader:GetInstance():LoadAsset(image_path, sprite_type, function(sprite, ...)
...
local function CoLoadImageAsync(self, atlas_config, image_name, progress_callback)
	local sprite = ResourcesManagerInst:CoLoadAsync(path, sprite_type, progress_callback)
