...
local function OnCreate(self, relative_order)
	...
	self.unity_graphic_raycaster = UIUtil.AddCSComponent(self.gameObject, typeof(CS.UnityEngine.UI.GraphicRaycaster))
	-- order
...
local function SetOrder(self, relative_order)
	...
	assert(relative_order < UIManagerInst.MaxOderPerWindow, "Relative order larger then MaxOderPerWindow!")
	self.relative_order = relative_order
	self.unity_canvas.sortingOrder = self.view.base_order + relative_order
end

local function SetLayerOrder(self, order)
	if self.unity_canvas then
		self.unity_canvas.sortingOrder = order
	end
end

local function GetLayerOrder(self)
	if self.unity_canvas then
...
UICanvas.SetLayerOrder = SetLayerOrder
UICanvas.GetLayerOrder = GetLayerOrder
