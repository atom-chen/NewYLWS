...
public class SimpleHttp : MonoBehaviour
{
    ...
    public static void HttpPost(string url, string msg, float timeOut = 10f)
    {
        HttpInfo httpInfo = new HttpInfo();
        httpInfo.callbackDel = null;
        httpInfo.url = url;
        httpInfo.formData = null;
        httpInfo.byteData = DataUtils.StringToBytes(msg);
    ...
    {
        m_info = info;
        m_www = new WWW(m_info.url, m_info.byteData);
        yield return m_www;
