...
namespace XLua
{
    public class OverloadMethodWrap
    {
        ...
        int[] outPosArray;
        int outPosArrayCount;

        bool[] isOptionalArray;
        int isOptionalArrayCount;

        object[] defaultValueArray;
        int defaultValueArrayCount;
        ...
        public void Init(ObjectCheckers objCheckers, ObjectCasters objCasters)
        {
            ...
            checkArray = new ObjectCheck[paramInfos.Length];
            castArray = new ObjectCast[paramInfos.Length];
            inPosArray = new int[paramInfos.Length];
            outPosArray = new int[paramInfos.Length];
            isOptionalArray = new bool[paramInfos.Length];
            defaultValueArray = new object[paramInfos.Length];
            ...
            for(int i = 0; i < paramInfos.Length; i++)
            {
				...
				{
					//outPosList.Add(i);
                    outPosArray[outPosArrayCount++] = i;
				}
                else
                {
                    if(paramInfos[i].ParameterType.IsByRef)
                    {
                        ...
                        {
                            refPos[i] = inPosArrayCount;        // inPosList.Count;
                        }
                        //outPosList.Add(i);
                        outPosArray[outPosArrayCount++] = i;
                    }
                    ...
                    checkArray[checkArrayCount++] = objCheckers.GetChecker(paramType);

                    //paramsCasts.Add (objCasters.GetCaster(paramType));
                    castArray[castArrayCount++] = objCasters.GetCaster(paramType);

                    //isOptionalList.Add(paramInfos[i].IsOptional);
                    isOptionalArray[isOptionalArrayCount++] = paramInfos[i].IsOptional;
                    ...
                    defaultValueArray[defaultValueArrayCount++] = paramInfos[i].IsOptional ? defalutValue : null;
                }
            }
        ...
        public bool Check(RealStatePtr L)
        {
            ...
            {
                if ((i == (checkArrayCount - 1)) && (paramsType != null))
                {
                    break;
                }
            ...
            return paramsType != null ? (luaStackPos < luaTop + 1 ? 
                checkArray[checkArrayCount - 1](L, luaStackPos) : true) : luaStackPos == luaTop + 1;
        }
        ...
        public int Call(RealStatePtr L)
        {
            try
            {
                ...
                for (int i = 0; i < castArrayCount; i++)
                {
                    ...
                    {
                        if (paramsType != null && i == castArrayCount - 1)
                        {
                ...
                for (int i = 0; i < outPosArrayCount; i++)
                {
