...
namespace XLua.LuaDLL
{
    ...
    public partial class Lua
	{
        ...
        private static int StringBufferLen = 512 * 1024;
        private static byte[] StringBuffer = new byte[StringBufferLen];
...
#else
                string ret = Marshal.PtrToStringAnsi(str, len);
                if (ret == null)
                {
                    byte[] buffer = StringBuffer;
                    Marshal.Copy(str, buffer, 0, len);
        ...
        [DllImport(LUADLL, CallingConvention = CallingConvention.Cdecl)]
        public static extern int xluaL_loadencbuffer(IntPtr L, byte[] buff, int size, string name);

        public static int luaL_loadencbuffer(IntPtr L, string buff, string name)//[-0, +1, m]
        {
            byte[] bytes = Encoding.UTF8.GetBytes(buff);
            return xluaL_loadencbuffer(L, bytes, bytes.Length, name);
        }

        [DllImport(LUADLL, CallingConvention = CallingConvention.Cdecl)]
        public static extern int xluaL_decode_encbuffer(byte[] buff, int size);
