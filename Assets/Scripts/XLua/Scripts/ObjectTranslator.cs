...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        Delegate getDelegate(DelegateBridgeBase bridge, Type delegateType)
        {
            ...
            {
                if (!item.IsConstructor && Utils.IsParamsMatch(delegateMethod, item))
                {
#if !UNITY_WSA || UNITY_EDITOR
                    return Delegate.CreateDelegate(delegateType, bridge, item);
#else
                    return item.CreateDelegate(delegateType, bridge); 
#endif
                }
            }
