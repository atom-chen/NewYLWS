...
/// <summary>
/// 这个管理器只负责加载lua脚本，其他的全部调用lua实现的AssetBundleMgr
/// </summary>
...
namespace AssetBundles
{
    ...
    public class AssetBundleMgr : MonoSingleton<AssetBundleMgr>
    {
        ...
        public void AddAssetbundleAssetsCache(string assetbundleName)
        {
            ...
            {
                Logger.LogError("AB have not yet been load, name:" + assetbundleName);
                return;
            }
            ...
            for (int i = 0; i < allAssetNames.Count; i++)
            {
                ...
                if (asset is ShaderVariantCollection)
                {
                    return;
                }
