...
namespace AssetBundles
{
    ...
    public class ResourceWebRequester : ResourceAsyncOperation
    {
        ...
        {
            if (url.EndsWith("assetbundle") && !url.Contains(BuildUtils.LuaABFileName))
            {
            ...
            {
                string error = www == null ? "_www_null" : www.error;
                Logger.LogError(string.Format("url: {0} error: {1}", url, error));
            }
            // 由update去设置对外的完成状态，因为协程的被执行的先后顺序不同，可能会造成AssetBundleMgr的update被后执行，而不能清空下载队列
            isLoadFinish = true;
        }
