...
public class FindImageInPrefabs : EditorWindow
{
    class RefInfo
    {
    ...
    [MenuItem("Tools/FindImageInPrefabs", false, 1)]
    static void Init()
    {
        FindImageInPrefabs window = EditorWindow.GetWindow < FindImageInPrefabs>("FindImageInPrefabs");
        window.Show();
    }
    ...
    private void FindFromPath(string findWhat)
    {
        ...
        string[] ids = AssetDatabase.FindAssets("t:Prefab", new string[] { "Assets/AssetsPackage/UI/Prefabs" });
        for (int i = 0; i < ids.Length; i++)
        {
            ...
            {
                if (images[j].mainTexture && images[j].mainTexture.name == (findWhat))      // && images[j].gameObject.name.EndsWith("_BTN") == false
                {

                    Debug.Log("------ " + images[j].mainTexture.name + " -- " +go.name + " , " + images[j].gameObject.name);
