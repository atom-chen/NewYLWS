...
public class PackageUtils
{
    ...
    public static string GetLocalServerIP()
    {
        ...
        {
            ip = EditorPrefs.GetString(LocalServerIPPrefsKey, ip);
        }
        return ip;
    }
    ...
    public static GAME_PLATFORM GetCurSelectedChannel()
    {
        GAME_PLATFORM platformType = GAME_PLATFORM.TEST;
        string channelName = EditorPrefs.GetString("ChannelName");
        if (Enum.IsDefined(typeof(GAME_PLATFORM), channelName))
        {
            platformType = (GAME_PLATFORM)Enum.Parse(typeof(GAME_PLATFORM), channelName);
        }
        else
        {
            EditorPrefs.SetString("ChannelName", GAME_PLATFORM.TEST.ToString());
        }
        return platformType;
    }

    public static void SaveCurSelectedChannel(GAME_PLATFORM platformType)
    {
        EditorPrefs.SetString("ChannelName", platformType.ToString());
    }
    ...
    {
        var buildTarget = EditorUserBuildSettings.activeBuildTarget;
        var channelName = GetCurSelectedChannel();
        return GetBuildPlatformOutputPath(buildTarget, channelName.ToString());
    }
    ...
    public static void CopyAssetBundlesToLocalServerPath(BuildTarget buildTarget, string channelName)
    {
        string source = GetBuildPlatformOutputPath(buildTarget, channelName);
        string destination = Path.Combine(AssetBundleConfig.AssetBundlesBuildOutputPath + "/FileDownloadServer", GetPlatformChannelPath(buildTarget, channelName));
        ...
        {
            Debug.LogError("Something wrong, you need manual delete AssetBundles folder in FileDownloadServer, err : " + ex);
            return;
        }
    ...
    public static void CopyAssetBundlesToStreamingAssets(BuildTarget buildTarget, string channelName)
    {
        ...
        string dontUpdateFilePath = AssetBundleUtility.GetStreamingAssetsDataPath(AssetBundleConfig.ABUpdateFileName);
        string[] contentArray = GameUtility.SafeReadAllLines(dontUpdateFilePath);
        if (contentArray != null && contentArray.Length >= 0)
        {
            for (int i = 0; i < contentArray.Length; i++)
            {
                string filePath = AssetBundleUtility.GetStreamingAssetsDataPath(contentArray[i]);
                GameUtility.SafeDeleteFile(filePath);
            }
        }

        string excludeListFilePath = AssetBundleUtility.GetStreamingAssetsDataPath(AssetBundleConfig.ExcludeFormApkFileName);
        string[] excludeArray = GameUtility.SafeReadAllLines(excludeListFilePath);
        if (excludeArray != null && excludeArray.Length >= 0)
        {
            for (int i = 0; i < excludeArray.Length; i++)
            {
                string filePath = AssetBundleUtility.GetStreamingAssetsDataPath(excludeArray[i]);
                GameUtility.SafeDeleteFile(filePath);
            }
        }
