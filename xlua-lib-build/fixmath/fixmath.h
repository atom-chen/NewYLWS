...
FX_EXPORT fixed_t 
fx_angle_to_radian(fixed_t xAngle);

FX_EXPORT fixed_t 
fx_radian_to_angle(fixed_t xRadian);


#include "fixmath_impl.h"  /* Internal implementations */
#include "fixmath_macro.h" /* Macro overrides          */

#define FRACT_LIMIT  8
#define F_IS_ZERO(f)  (((f)>0?(f):-(f))<1e-8)
#define FX_ZERO (fx_dtox(0.000, FRACT_LIMIT))
#define FX_IS_ZERO(fx) ((fx) == fx_dtox(0.000, FRACT_LIMIT))
#define FX_ABS(fx) ( ((fx) > fx_dtox(0.000, FRACT_LIMIT)) ? fx : fx_subx(fx_dtox(0.000, FRACT_LIMIT), fx))
#define FX_DTOX(x) (fx_dtox(x, FRACT_LIMIT))
#define FX_XTOD(x) (fx_xtod(x, FRACT_LIMIT))
#define FX_ADDX(a, b) (fx_addx(a, b))
#define FX_SUBX(a, b) (fx_subx(a, b))
#define FX_MULX(a, b) (fx_mulx(a, b, FRACT_LIMIT))
#define FX_DIVX(a, b) (fx_divx(a, b, FRACT_LIMIT))
