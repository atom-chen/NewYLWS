...
LUALIB_API int xluaL_loadencbuffer (lua_State *L, const char *buff, int size,
                                const char *name) {
	return luaL_loadencbuffer(L, buff, size, name);
}


#define XXTEA_DELTA 0x9ebca9b9
#define XXTEA_MX (((z>>5^y<<2) + (y>>3^z<<4)) ^ ((sum^y) + (key[(p&3)^e] ^ z)))
static void xxtea_dec(char *data, int len) {
...
LUALIB_API int xluaL_decode_encbuffer (const char* buff, size_t size) {
  /// xor 解密
  //for ( int i = 0; i < size; ) {
  //  if (i + 2 > size) {
  //    break;
  //  }
  //  *(unsigned short*)(buff + i) = (*(unsigned short*)(buff + i))^0xaedf;  /// 解密
  //  i += 2;
  //}
  xxtea_dec((char*)buff, (int)size);
  return 0;
}
